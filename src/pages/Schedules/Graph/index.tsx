...
export const GraphSchedule: React.FC<GraphScheduleProps> = ({ holidays, selectedItems, setSelectedItems, dutiesOptions, defaultTime, today, setDefaultTime, items}) => {
  ...
  const itemRenderer = ({item, getItemProps}) => {
    ...
    const canClick = !item.isDeactivated && !item.isEmpty
    return (
  ...
  return (
    <>
      <div className={styles.table}>
        <Timeline
          groups={dutiesOptions}
          items={formItems || []}
          minZoom={60 * 60 * 24 * 1000}
          maxZoom={30 * 24 * 60 * 60 * 1000 * 12}
        ...
        >
          <TimelineHeaders>
            <SidebarHeader>
              {({ getRootProps }) => {
                return <div {...getRootProps()} className={styles.title}>{dutiesOptions ? 'Дежурный' : 'Группа'}</div>
              }}
            </SidebarHeader>
